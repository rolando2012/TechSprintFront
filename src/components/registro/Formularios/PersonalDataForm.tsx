...
const personalDataSchema = z.object({
  nombre: z.string()
    ...
    .regex(/^[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ\s]+$/, { message: 'El nombre solo debe contener caracteres alfabéticos' }),
  apellido: z.string()
    .min(1, { message: 'El apellido es requerido' })
    .regex(/^[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ\s]+$/, { message: 'El apellido solo debe contener caracteres alfabéticos' }),
  carnetIdentidad: z.string()
    .min(1, { message: 'El carnet de identidad es requerido' })
    .regex(/^[a-zA-Z0-9]+$/, { message: 'El carnet no debe contener caracteres especiales' }),
  ...
  grado: z.string().min(1, { message: 'El grado es requerido' }),
  nivel: z.string().min(1, { message: 'El nivel es requerido' }),
...
const PersonalDataForm = forwardRef<HTMLFormElement, Props>(({ onSubmitSuccess }, ref) => {
  ...
  // Update municipios when departamento changes
  useEffect(() => {
    if (formData.departamento) {
      setMunicipios(getMunicipiosByDepartamento(formData.departamento));
  ...
  // Update niveles when grado changes
  useEffect(() => {
    if (formData.grado) {
      setNiveles(getNivelesByGrado(formData.grado));
      // Reset nivel if grado changes
        ...
        <div className="relative">
          <div className="flex items-center">
            ...
            <select
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              className={`flex-1 border-b-2 bg-transparent py-2 pl-2 focus:outline-none ${errors.nivel ? 'border-red-500' : 'border-gray-300'}`}
