...
export default function AdministradorPage() {
  const features = [
    ...
    { icon: <FaClipboardList size={75} />, label: 'Lista de competidores' },
    { icon: <FaUsersCog size={75} />, label: 'Gestionar Competencia' },
  ]
    ...
    <main className="min-h-screen bg-gray-100">
      ...
      <section className="bg-[#e4e4e4] py-10 px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 place-items-center">
        {features.map(({ icon, label }) => (
          <div
            key={label}
            className="bg-[#B4B9C5] rounded-xl p-6 w-full max-w-[220px] h-[200px] shadow hover:shadow-md transition flex flex-col justify-center items-center text-center"
          >
